...
chart = chart_regional_cap(cap,width=alt.Step(60))
chart
```
```{python}
# | title: Regional New-buid Capacity
chart = chart_regional_cap(cap.loc[cap["new_build"] == True, :],width=alt.Step(60))
chart
```
```{python}
# | title: Regional Capacity only NY
cap_only_ny = cap[cap['zone'].str.contains('NY', na=False)]
chart = chart_regional_cap(cap_only_ny,width=alt.Step(60))
chart
```
```{python}
# | title: Regional New-buid Capacity only NY
cap_only_ny = cap[cap['zone'].str.contains('NY', na=False)]
chart = chart_regional_cap(cap_only_ny.loc[cap_only_ny["new_build"] == True, :],width=alt.Step(60))
chart
```
#### Column {.tabset}
```{python}
...
chart = chart_regional_gen(gen, cap,width=alt.Step(60))
chart
```
```{python}
...
chart = chart_regional_gen(gen_only_ny, cap_only_ny,width=alt.Step(60))
chart
```
...
chart = chart_emissions_intensity(emiss, gen, x_var="model", col_var="Region", width=alt.Step(60),)
chart
```
```{python}
...
from fig_functions_genx_only import chart_emissions_intensity
emiss_only_ny = emiss[emiss['Region'].str.contains('NY', na=False)]
chart = chart_emissions_intensity(emiss_only_ny, gen, x_var="model", col_var="Region",width=alt.Step(60),)
chart
```
# Dispatch
...
chart = chart_dispatch_single_tech(data)
chart
```
```{python}
