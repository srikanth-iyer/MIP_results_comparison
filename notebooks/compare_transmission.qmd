---
title: "Compare transmission results"
format: dashboard
jupyter: python3
categories:
    - Full
...
chart = chart_total_cap(avg_total_cap.reset_index(), row_var=None, col_var="planning_year", x_var="case", order=order)
chart
```
```{python}
# | title: New-build Capacity
total_new_cap = cap.query("new_build == True").groupby(["case", "planning_year", "tech_type", "model"], as_index=False)["end_value"].sum()
mean_new_cap = total_new_cap.groupby(["case", "planning_year", "tech_type"], as_index=False)["end_value"].agg(["mean", "min", "max"]).rename(columns={"mean": "end_value"})
chart = chart_total_cap(mean_new_cap, row_var=None, col_var="planning_year", x_var="case", order=order)
chart
```
```{python}
