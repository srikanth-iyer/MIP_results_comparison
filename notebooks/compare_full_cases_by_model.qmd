---
title: "Compare 52-week scenario results by model"
format: dashboard
#   dashboard:
#     scrolling: false
jupyter: python3
...
order = [f.replace("full-", "") for f in data_folders]
...
chart = chart_total_cap(cap, col_var="model", x_var="case", order=order)
chart
```
```{python}
# | title: New-build Capacity
chart = chart_total_cap(
    cap.loc[cap["new_build"] == True, :], col_var="model", x_var="case", order=order
)
chart
```
```{python}
...
chart = chart_total_cap(
    cap.loc[cap["new_build"] == False, :], col_var="model", x_var="case", order=order
)
chart
```
...
```{python}
chart = chart_total_gen(gen, cap, col_var="model", x_var="case", order=order)
chart
```
...
```{python}
chart = chart_emissions(emiss, col_var="model", x_var="case", order=order)
chart
```
...
chart = chart_tx_expansion(
    tx_all, col_var="case", facet_col=None, x_var="line_name", row_var="model", order=order
)
chart
```
...
chart = chart_op_cost(op_costs, col_var="case", x_var="model", row_var="planning_year", order=order)
chart
```
```{python}
# | title: Operational NSE
chart = chart_op_nse(op_nse, col_var="case", x_var="model", row_var="planning_year", order=order)
chart
```
