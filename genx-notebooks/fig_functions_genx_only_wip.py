...
from fig_chart_utils import (
    VAR_ABBR_MAP,
    VAR_ABBR_TITLE_MAP,
    configure_full_label_display,
    tech_color_encoding,
    title_case,
)
...
rev_region_map = reverse_region_map()
...
def load_genx_operations_data(
    ...
    if "Resource" in df.columns:
        ...
        zone_series = df["resource_name"].astype(str).str.split("_").str[0]
        df.loc[:, "zone"] = zone_series
        ...
        for year in data["planning_year"].unique():
            ...
            if year == first_year:
                ...
                if first_data.empty:
                    ...
                    reference = first_data["value"].reindex(_df.index)
                    line_growth = _df["value"].div(reference)
                    _df["line_growth"] = (
                        line_growth.replace([np.inf, -np.inf], np.nan).fillna(0)
                    )
            df_list.append(_df)
...
def _candidate_geojson_paths(filename: str) -> List[Path]:
    base_dir = Path(__file__).parent
    workspace_dir = Path.cwd()
    return [
        base_dir / filename,
        workspace_dir / filename,
        workspace_dir / "notebooks" / filename,
        Path("/Users/gs5183/Documents/MIP_results_comparison/notebooks") / filename,
    ]


def load_tx_geometries(filename: str = "conus_26z_latlon_simple.geojson") -> gpd.GeoDataFrame:
    """Load transmission topology geometries with reasonable fallback paths."""

    for candidate in _candidate_geojson_paths(filename):
        if candidate.exists():
            gdf = gpd.read_file(candidate)
            return gdf.rename(columns={"model_region": "zone"})
    searched = ", ".join(str(p) for p in _candidate_geojson_paths(filename))
    raise FileNotFoundError(
        f"Could not locate {filename}. Checked: {searched}"
    )
...
def chart_cap_factor_scatter( 
    ...
    if dispatch is not None:
        even_hours_mask = (dispatch["hour"] < 120) & (dispatch["hour"] % 2 == 0)
        _dispatch = dispatch.loc[even_hours_mask]
...
def chart_cost_mwh(
    ...
    chart = config_chart_row_col(chart, row_var, col_var, x_var)
    return chart
def agg_region_map(geojson: gpd.GeoDataFrame | None = None) -> alt.Chart:
    gdf = geojson.copy() if geojson is not None else load_tx_geometries()
